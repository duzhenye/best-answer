...
namespace Duzhenye\BestAnswer\Repository;
...
class BestAnswerRepository
{
    ...
    public function canSelectPostAsBestAnswer(User $user, Post $post): bool
    {
        ...
        if ($user->id === $post->user_id) {
            return (bool) $this->settings->get('duzhenye-best-answer.allow_select_own_post');
        }
    ...
    public function setBestAnswer(Discussion $discussion, User $actor, int $id): void
    {
        ...
        if ($id && !$post) {
            throw new ValidationException(
                [
                    'error' => $this->translator->trans('duzhenye-best-answer.forum.errors.mismatch'),
                ]
            );
        }
    ...
    {
        $tagsToChange = @json_decode($this->settings->get('duzhenye-best-answer.select_best_answer_tags'));
